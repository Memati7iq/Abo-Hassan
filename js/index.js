...
function CheckFW() {
  ...
  if (ps4Regex.test(userAgent)) {
    ...
    document.title = "Abo Hassan | " + fwVersion;
  } else {
...
const goldhenFiles = [
  'goldhen_2.2.3_900.bin',
  'goldhen_2.2.3_672.bin',
  'goldhen_2.2.3_505.bin',
  'goldhen_2.2.2_900.bin',
  'goldhen_2.2.2_755.bin',
...
let goldHENRendered = false;

function showgoldhenpayloads() {
  const container = document.getElementById("payloads-goldhen");
  if (container.style.display === "none") {
    container.style.display = "block";
    if (!goldHENRendered) {
      renderGoldHENPayloads();
      goldHENRendered = true;
    }
  } else {
...
function renderGoldHENPayloads() {
  const buttonContainer = document.querySelector("#payloads-goldhen .button-container");
  buttonContainer.innerHTML = ""; // تفريغ المحتوى

  if (!ps4fw) {
    buttonContainer.innerHTML = "<p>⚠️ تعذر تحديد إصدار النظام.</p>";
    return;
  }

  const filteredFiles = goldhenFiles.filter(file => {
    const fwMatch = file.match(/_(\d{3})\.bin$/);
    return fwMatch && fwMatch[1] === ps4fw;
  });

  if (filteredFiles.length === 0) {
    buttonContainer.innerHTML = `<p>❌ لا يوجد بايلود GoldHEN متوافق مع النظام ${ps4fw.slice(0,1)}.${ps4fw.slice(1)}.</p>`;
    return;
  }

  filteredFiles.forEach(file => {
    const match = file.match(/goldhen_([\d\.a-z]+)_(\d{3})\.bin/i);
    if (match) {
      const version = match[1]; // مثل 2.3 أو 2.4b17.3
      const fw = `${match[2].charAt(0)}.${match[2].slice(1)}`;
      const btn = document.createElement("button");
      btn.textContent = `GoldHEN ${version} - ${fw}`;
      btn.onclick = () => loadGoldHEN(file);
...
function loadGoldHEN(filename) {
  consoleDev.append(`Loading GoldHEN payload: ${filename}\n`);
  consoleDev.scrollTop = consoleDev.scrollHeight;

  fetch(`payloads/${filename}`)
    .then(() => {
      consoleDev.append(`Payload ${filename} loaded successfully!\n`);
      consoleDev.scrollTop = consoleDev.scrollHeight;
    })
    .catch(error => {
      consoleDev.append(`Error loading ${filename}: ${error}\n`);
      consoleDev.scrollTop = consoleDev.scrollHeight;
    });
}